...
impl Session {
    ...
    // ALTERADO PELO MORUNE -- ver vendor/README.md.
    //
    // O original chama `process::exit(1)` aqui. Como este metodo roda no
    // manipulador do pacote de produto, uma conta gratuita fazia o processo
    // inteiro desaparecer da tela, sem erro, sem log e sem janela. Num
    // aplicativo aberto, onde qualquer pessoa baixa e clica em "Entrar", esse e
    // o primeiro contato da maioria das contas do Spotify com o Morune.
    //
    // Aqui o plano so e registrado. Quem decide o que fazer e o aplicativo, que
    // le `get_user_attribute("type")` depois de conectar e explica a situacao em
    // vez de sumir. A reproducao falha sozinha, como ja falharia.
    fn check_catalogue(attributes: &UserAttributes) {
        if let Some(account_type) = attributes.get("type") {
            if account_type != "premium" {
                error!("conta {account_type:?} nao recebe audio do Spotify");
            }
        }
    }
